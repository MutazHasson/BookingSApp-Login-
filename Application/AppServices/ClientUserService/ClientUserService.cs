...
namespace Application.AppServices.ClientUserService.DTOs
{
    public class ClientUserService : IClientUserService
    {
        ...
        public async Task UpdateClientUserAccount(ClientUserRegisterationRequest request)
        {
            ...
            var clientUser = await _clientUserRepo.GetAll().FirstOrDefaultAsync(x => x.UserId == userId);  // Get the client user from the database by user id
            if (clientUser == null)
            {
                throw new Exception("Client user not found");
            }
            ...
            clientUser.BirthDate = request.BirthDate;  // Update the birth date of the client user
            _clientUserRepo.Update(clientUser);  // Update the client user in the database
            await _clientUserRepo.SaveChangesAsync();  // Save the changes to the database
